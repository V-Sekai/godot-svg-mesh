	...
	void write(float *p) {
		switch (type) {
			...
			case RECT_ROUND: {
				...
				*p++ = y;
				p = lineTo(p, x + w - rx, y);
				*p++ = x + w - rx * (1 - TOVE_KAPPA90);
				*p++ = y;
				*p++ = x + w;
				*p++ = y + ry * (1 - TOVE_KAPPA90);
				*p++ = x + w;
				*p++ = y + ry;
				p = lineTo(p, x + w, y + h - ry);
				*p++ = x + w;
				*p++ = y + h - ry * (1 - TOVE_KAPPA90);
				*p++ = x + w - rx * (1 - TOVE_KAPPA90);
				*p++ = y + h;
				*p++ = x + w - rx;
				*p++ = y + h;
				p = lineTo(p, x + rx, y + h);
				*p++ = x + rx * (1 - TOVE_KAPPA90);
				*p++ = y + h;
				*p++ = x;
				*p++ = y + h - ry * (1 - TOVE_KAPPA90);
				*p++ = x;
...
public:
	...
	EllipsePrimitive(float cx, float cy, float rx, float ry) :
	...
	void write(float *p) {
		...
		*p++ = cy;
		*p++ = cx + rx;
		*p++ = cy + ry * TOVE_KAPPA90;
		*p++ = cx + rx * TOVE_KAPPA90;
		*p++ = cy + ry;
		*p++ = cx;
		*p++ = cy + ry;
		*p++ = cx - rx * TOVE_KAPPA90;
